...
import { Movie } from './Api'; // Import Movie interface
...
const MovieDetail: React.FC<MovieDetailProps> = ({ onClose = () => {} }) => {
  ...
  const navigate = useNavigate(); // Add navigate for closing
  ...
  useEffect(() => {
    const fetchMovieDetail = async () => {
      ...
      try {
        setLoading(true);
        const data = await getMovieById(Number(id)); // Ensure correct type
        setMovie(data);
        setError(null);
      } catch (err: any) {
    ...
    fetchMovieDetail();
  }, [id]); // Refetch when the movie ID changes
  ...
  const handleBackdropClick = (e: React.MouseEvent) => {
    if (e.target === e.currentTarget) {
      console.log('Backdrop clicked, calling onClose and navigating'); // Debug log
      onClose();
      navigate('/home'); // Navigate back to Dashboard
    }
  };
  ...
  const handleClose = (e: React.MouseEvent) => {
    ...
    console.log('Close button clicked, calling onClose and navigating'); // Debug log
    onClose();
    navigate('/home'); // Navigate back to Dashboard
  };
  ...
  return (
    <div
      ...
      style={{
        ...
        WebkitBackdropFilter: 'blur(120px)', // For Safari compatibility
