...
// import { createSubscription } from '../subApi';
...
//       if (!token) {
//         setError('You need to sign in to subscribe.');
...
//       // Allow subscription if no active subscription or if the current plan is 'basic'
//       if (status.is_active && status.plan_type !== 'basic') {
...
//       const errorMessage = err.message || 'Failed to initiate subscription. Please try again.';
    ...
    return (
      <Box sx={{ bgcolor: 'white', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
        <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', p: 3 }}>
          ...
          >
            <Box sx={{ width: 64, height: 64, bgcolor: '#3a8dff', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', mx: 'auto', mb: 3 }}>
              <CheckCircle color="primary" sx={{ fontSize: 36 }} />
            </Box>
            <Typography variant="h4" component="h2" gutterBottom>
              Subscription Activated!
            </Typography>
            <Typography variant="body1" color="rgba(0, 0, 0, 0.7)" gutterBottom sx={{ mb: 3 }}>
              Thank you for subscribing to Movie Explorer. Your {plans.find((p) => p.id === selectedPlan)?.name} (${plans.find((p) => p.id === selectedPlan)?.price}) has been activated.
            </Typography>
            <Button
              variant="contained"
              ...
              fullWidth
              onClick={() => (window.location.href = '/dashboard')}
              sx={{ bgcolor: '#3a8dff', '&:hover': { bgcolor: '#1e60b7' } }}
            >
              Start Exploring Movies
            </Button>
          </Paper>
  ...
  return (
    <Box sx={{ bgcolor: 'white', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      <Box sx={{ flex: 1, py: 6 }}>
        <Container maxWidth="lg">
          <Box sx={{ textAlign: 'center', mb: 6 }}>
            <Typography variant="h3" component="h1" fontWeight="bold" gutterBottom sx={{ color: '#000' }}>
              Choose Your Movie Explorer Plan
            </Typography>
                ...
                <CardActions sx={{ p: 3, pt: 0 }}>
                  <Button
                    ...
                    sx={{
                      color: selectedPlan === plan.id ? '#fff' : '#000',
                      borderColor: '#3a8dff',
                      bgcolor: selectedPlan === plan.id ? '#3a8dff' : 'transparent',
                      '&:hover': { bgcolor: selectedPlan === plan.id ? '#1e60b7' : 'rgba(58, 141, 255, 0.1)', borderColor: '#3a8dff' },
                    }}
                  >
                    {selectedPlan === plan.id ? 'Selected' : 'Select Plan'}
                  </Button>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'center' }}>
              ...
              >
                ...
                <Button
                  ...
                  disabled={isProcessing}
                  onClick={handleSubscribe}
                  sx={{ py: 1.5, bgcolor: '#3a8dff', '&:hover': { bgcolor: '#1e60b7' } }}
                >
                  {isProcessing ? (
                    <Box sx={{ display: 'flex', alignItems: 'center' }}>
                      <CircularProgress size={24} color="inherit" sx={{ mr: 1 }} />
                ...
                <Typography variant="body2" color="rgba(0, 0, 0, 0.7)" align="center" sx={{ mt: 2 }}>
                  You can cancel your subscription at any time from your account settings
                </Typography>
              </Paper>
            </Box>
