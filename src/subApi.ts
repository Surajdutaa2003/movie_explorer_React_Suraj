...
export const getSubscriptionStatus = async (token: string): Promise<{ is_active: boolean; plan_type?: string }> => {
  try {
    if (!token) {
      toast.error('You need to sign in first.');
      throw new Error('No authentication token found');
    ...
    const responseData = response.data as SubscriptionStatusResponse;

    if (responseData.error) {
      throw new Error(responseData.error);
    }

    // Assume subscription is active if plan_type is present (e.g., "premium")
    const isActive = !!responseData.plan_type;
    ...
    console.error('Subscription Status Error:', {
      ...
      status: error.isAxiosError ? error.response?.status : undefined,
    });
